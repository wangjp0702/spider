...
for i in range(1,30):
    driver.find_element_by_id('wxmore').click()
    time.sleep(3)
    ##data=driver.find_element_by_id('wxbox').text
    result=driver.page_source
    soup = BeautifulSoup(result)
    print(soup.find("title"))
    print(soup.find(class_='res-login').string)
    count=0
    for link in soup.findAll(class_='wx-rb wx-rb3'):
        content=link.find('div',class_='txt-box')
        url=content.find('a').get('href')
        title=content.find('a').string
        summary=content.find('p').string
        #print(url+"\n\r"+title+"\n\r"+summary+"\n\r")
        count=count+1
